...
function createMovieCard(movie, { onUnmarkWatched }) {
  ...
  moreBtn.href = `movie.html?id=${movie.id}&from=watched`;
  row.appendChild(moreBtn);

  const removeBtn = document.createElement("button");
  removeBtn.className = "btn danger";
  removeBtn.textContent = "Remove";
  removeBtn.addEventListener("click", () => onUnmarkWatched(movie.id));
...
async function init() {
  ...
  const watchedIds = getWatched(user.id);
  const watchedMovies = merged.filter(m => watchedIds.includes(m.id));

  if (!moviesContainer) return;
  moviesContainer.innerHTML = "";

  if (!watchedMovies.length) {
    moviesContainer.innerHTML = "<p>No watched movies yet.</p>";
    return;
  }

  const onUnmarkWatched = (movieId) => {
    const u = requireAuth();
    if (!u) return;

    const list = getWatched(u.id).filter(id => Number(id) !== Number(movieId));
    setWatched(u.id, list);
    window.location.reload();
  };

  watchedMovies.forEach(m => moviesContainer.appendChild(createMovieCard(m, { onUnmarkWatched })));
}

init().catch(err => {
  console.error(err);
  if (moviesContainer) moviesContainer.innerHTML = "<p>Failed to load watched movies.</p>";
});
