...
const clearBtn = document.getElementById("clearWatchedBtn");
...
function createCard(movie) {
  ...
  more.href = `movie.html?id=${encodeURIComponent(movie.id)}&from=watched`;
  ...
  removeBtn.addEventListener("click", () => {
    let ids = getIds(KEYS.WATCHED, userId);
    ids = ids.filter((id) => id !== movie.id);
    setIds(KEYS.WATCHED, userId, ids);
    render();
  });
...
function render() {
  ...
  const ids = getIds(KEYS.WATCHED, userId);
  const watchedMovies = mergedMovies.filter((m) => ids.includes(m.id));

  const q = searchInput ? searchInput.value : "";
  const filtered = filterBySearch(watchedMovies, q);
  const sorted = sortByTitleAsc(filtered);

  grid.innerHTML = "";

  if (sorted.length === 0) {
    grid.innerHTML = `<div class="card"><div class="card-inner">No watched movies yet.</div></div>`;
    return;
  }
...
async function init() {
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      if (!confirm("Clear all watched movies?")) return;
      setIds(KEYS.WATCHED, userId, []);
      render();
    });
  }
...
init().catch((err) => {
  console.error(err);
  if (grid) grid.innerHTML = `<div class="card"><div class="card-inner">Error loading watched.</div></div>`;
});
